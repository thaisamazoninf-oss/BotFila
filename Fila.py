...
with sync_playwright() as p:
    ...
    page.click("body > app-root > app-layout > ng-sidebar-container > div > div > app-chat-home > app-content > main > div > div > div > div.chats-nav.ng-star-inserted > div.chat_item_header.types-bot > span:nth-child(2)")
    ...
    while page.locator(".protocol.gupshup:not(.ativo)").count()> 0:
        try:
            ...
            if total > ultimo_total:
                ...
                for i in range(ultimo_total, total):
                    ...
                    #----------Se for Público Geral -----------------#
                    if tipo_usuario == "PGeral" and not ja_imprimiu:
                        
                        cpf_encontrado = re.findall(cpf_pattern, texto_limpo)
                        
                        if cpf_encontrado:
                            cpf = cpf_encontrado.group()
                            
                        print("usuario: Público em Geral")
                        print(f"CPF:{cpf}")
                        print("-"*20)
